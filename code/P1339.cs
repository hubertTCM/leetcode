...
namespace P1339
{
    public class Solution
    {
        public int MaxProduct(TreeNode root)
        {
            ...
            var closest = 0L;
            CalculateCloset(root, total, ref closest);
            var mode = 1000000007;
            return Convert.ToInt32(((closest % mode) * ((total - closest) % mode)) % mode);
        }

        int CalculateTotal(TreeNode root)
        {
            if (root == null)
            {
                return 0;
            }
            var total = CalculateTotal(root.left) + CalculateTotal(root.right) + root.val;
            return total;
        }

        int CalculateCloset(TreeNode root, int total, ref long closest)
        {
            if (root == null)
            {
                return 0;
            }
            var left = CalculateCloset(root.left, total, ref closest);
            var right = CalculateCloset(root.right, total, ref closest);
            var temp = left + right + root.val;
